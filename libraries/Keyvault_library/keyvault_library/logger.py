"""Logging setup for keyvault_library."""
...
from libraries.Keyvault_library.keyvault_library.exceptions import LoggerSetupError

LOGGER_NAMESPACE = "keyvault_library"
LOG_FILE_PREFIX = "keyvault_library"


def get_bootstrap_logger() -> logging.Logger:
    """Return a package logger used before file logging is configured."""
...
def get_log_file_name() -> str:
    """Return the UTC-dated log filename."""

    return f"{LOG_FILE_PREFIX}_{datetime.now(UTC):%Y%m%d}.log"


def create_file_logger(log_location: str | Path) -> logging.Logger:
    """Create a structured file logger.

    Args:
        log_location: Directory where log files should be written.

    Returns:
        Logger configured with one file handler.
    ...
    try:
        ...
        raise LoggerSetupError("Unable to initialize keyvault_library logger.") from exc
        ...
        raise LoggerSetupError(
            "Invalid keyvault_library logger configuration."
        ) from exc


def _build_formatter() -> logging.Formatter:
    """Build an audit-friendly structured formatter."""
